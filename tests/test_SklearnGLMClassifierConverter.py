...
class TestGLMClassifierConverter(unittest.TestCase):
    ...
    def test_model_logistic_regression_binary_class(self):
        ...
        model_onnx = to_onnx(model, 'logistic regression', [
    ...
    def ztest_model_logistic_regression_multi_class(self):
        model, X = self._fit_model_multiclass_classification(
            linear_model.LogisticRegression())
        model_onnx = to_onnx(model, 'maximum entropy classifier', [
                             ('input', FloatTensorType([1, 3]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model, model_onnx,
    ...
    def test_model_linear_svc_binary_class(self):
        ...
        model_onnx = to_onnx(model, 'linear SVC', [
                             ('input', FloatTensorType([1, 3]))])
        self.assertIsNotNone(model_onnx)
    ...
    def test_model_linear_svc_multi_class(self):
        ...
        model_onnx = to_onnx(model, 'multi-class linear SVC',
                             [('input', FloatTensorType([1, 3]))])
        self.assertIsNotNone(model_onnx)
    ...
    def test_model_sgd_binary_class(self):
        ...
        model_onnx = to_onnx(
            model, 'scikit-learn SGD binary classifier', [('input', FloatTensorType([1, 3]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model, model_onnx,
