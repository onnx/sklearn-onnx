...
import pandas
import warnings
from .utils_backend import load_data_and_model, extract_options
from .utils_backend import ExpectedAssertionError, OnnxRuntimeAssertionError
from .utils_backend import compare_outputs


def compare_runtime(test, decimal=5, options=None, verbose=False,
    ...
    if DF:
        ...
        if isinstance(input, dict):
            ...
            if len(inp) == len(input):
                ...
                shape = sum(i.shape[1] if len(i.shape) ==
                            2 else i.shape[0] for i in inp)
                ...
                if shape == array_input.shape[1]:
                    ...
                    raise OnnxRuntimeAssertionError(
                        "Wrong number of inputs onnx {0}={1} columns != original shape {2}, onnx='{3}'*".format(
                            len(inp), shape, array_input.shape, onx))
            else:
                raise OnnxRuntimeAssertionError(
                    "Wrong type of inputs onnx {0}, onnx='{2}'".format(type(input), onx))
        else:
            raise OnnxRuntimeAssertionError(
    ...
    if OneOff or OneOffArray:
        ...
        if len(inputs) == 1 and not OneOffArray:
            ...
            for input in values:
                try:
                    ...
                    if lambda_onnx is None:
                        def lambda_onnx(): return sess.run(None, {name: input})
                    if verbose:
                        import pprint
                        pprint.pprint(one)
                except ExpectedAssertionError as expe:
            ...
            for i in range(0, len(t[1])):
                ...
                try:
                    ...
                    if lambda_onnx is None:
                        def lambda_onnx(): return sess.run(None, iii)
                    if verbose:
                        import pprint
                        pprint.pprint(one)
                except ExpectedAssertionError as expe:
                    ...
                    if verbose:
                        ...
                        smodel = ""
                    raise OnnxRuntimeAssertionError(
                        "Unable to run onnx '{0}' due to {1}{2}".format(onx, e, smodel))
                res.append(one)
            if verbose:
        ...
        try:
            ...
            def lambda_onnx(): return sess.run(None, inputs)
            if verbose:
                import pprint
            ...
            if "-Fail" in onx:
                ...
                if verbose:
                    ...
                    smodel = ""
                raise OnnxRuntimeAssertionError(
                    "onnxruntime cannot compute the prediction for '{0}' due to {1}{2}".format(
                        onx, e, smodel))
        except Exception as e:
            raise OnnxRuntimeAssertionError(
                "Unable to run onnx '{0}' due to {1}".format(onx, e))
    ...
    if isinstance(expected, list):
        if isinstance(output, list):
            ...
            for exp, out in zip(expected, output):
                _compare_expected(exp, out, sess, onnx,
                                  decimal=5, verbose=verbose, **kwargs)
                tested += 1
        else:
            raise OnnxRuntimeAssertionError(
