...
class TestGLMRegressorConverter(unittest.TestCase):
    ...
    def test_model_linear_regression(self):
        ...
        model_onnx = to_onnx(model, 'linear regression', [
                             ('input', FloatTensorType([1, 4]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model,
                            model_onnx, basename="SklearnLinearRegression-Dec4")
    ...
    def test_model_linear_svr(self):
        ...
        model_onnx = to_onnx(model, 'linear SVR', [
                             ('input', FloatTensorType([1, 4]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model,
                            model_onnx, basename="SklearnLinearSvr-Dec4")

    def test_model_ridge(self):
        model, X = self._fit_model(linear_model.Ridge())
        model_onnx = to_onnx(model, 'ridge regression', [
                             ('input', FloatTensorType([1, 4]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model,
                            model_onnx, basename="SklearnRidge-Dec4")

    def test_model_sgd_regressor(self):
        model, X = self._fit_model(linear_model.SGDRegressor())
        model_onnx = to_onnx(model, 'scikit-learn SGD regression',
                             [('input', FloatTensorType([1, 4]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model,
                            model_onnx, basename="SklearnSGDRegressor-Dec4")

    def test_model_elastic_net_regressor(self):
        model, X = self._fit_model(linear_model.ElasticNet())
        model_onnx = to_onnx(
            model, 'scikit-learn elastic-net regression', [('input', FloatTensorType([1, 4]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model,
                            model_onnx, basename="SklearnElasticNet-Dec4")

    def test_model_lasso_lars(self):
        model, X = self._fit_model(linear_model.LassoLars(alpha=0.01))
        model_onnx = to_onnx(model, 'lasso lars', [
                             ('input', FloatTensorType([1, 4]))])
        self.assertIsNotNone(model_onnx)
        dump_data_and_model(X.astype(numpy.float32), model,
                            model_onnx, basename="SklearnLassoLars-Dec4")
