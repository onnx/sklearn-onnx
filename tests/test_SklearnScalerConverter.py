...
class TestSklearnScalerConverter(unittest.TestCase):
    ...
    def test_standard_scaler(self):
        ...
        model.fit(data)
        model_onnx = convert(model, 'scaler', [('input', Int64TensorType([1, 3]))])
    ...
    def test_standard_scaler_floats(self):
        ...
        data = [[0., 0., 3.], [1., 1., 0.], [0., 2., 1.], [1., 0., 2.]]
        model.fit(data)
        model_onnx = convert(model, 'scaler', [('input', FloatTensorType([1, 3]))])
        self.assertTrue(model_onnx is not None)
    ...
    def test_min_max_scaler(self):
        ...
        data = [[0., 0., 3.], [1., 1., 0.], [0., 2., 1.], [1., 0., 2.]]
        model.fit(data)
        model_onnx = convert_convertsklearn(model, 'scaler', [('input', FloatTensorType([1, 3]))])
        self.assertTrue(model_onnx is not None)
        dump_data_and_model(numpy.array(data, dtype=numpy.float32),
