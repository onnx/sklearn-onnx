...
from ..algebra.onnx_ops import (
    OnnxWhere, OnnxIsNaN, OnnxNot, OnnxCast, OnnxEqual, OnnxNeg,
    OnnxSub, OnnxMul, OnnxAdd, OnnxDiv, OnnxGather, OnnxLessOrEqual,
    OnnxReshape, OnnxShape, OnnxFlatten, OnnxGreaterOrEqual, OnnxOr
)
...
def convert_quantile_transformer(scope: Scope,
                                 operator: Operator,
    ...
    quantiles_neg_rev_pad = _pad_quantiles(quantiles_neg_rev, target_size, n_quantiles, 'min')
    references_neg_rev_pad = _pad_references(references_neg_rev, target_size, n_quantiles, 'min')
    ...
    references_name = prepare_initializer(np.tile(references_pad, n_features), 'references')
    quantiles_neg_rev_name = prepare_initializer(quantiles_neg_rev_pad, 'quantiles_neg_rev', transpose=True)
    references_neg_rev_name = prepare_initializer(np.tile(references_neg_rev_pad, n_features), 'references_neg_rev')
    ...
    def create_interpolation(X_input, quantiles_init, references_init):
        ...
        current_index = OnnxMul(OnnxCast(X_input, to=np.int64, op_version=opv), 
                               np.array(0, dtype=np.int64), op_version=opv)
        current_index = OnnxFlatten(
            OnnxAdd(current_index, 
                    np.array([i * (target_size + 1) for i in range(n_features)], dtype=np.int64), 
                    op_version=opv), 
            axis=0, op_version=opv)
        current_index.add_to(scope, container)

        step = OnnxFlatten(OnnxAdd(OnnxMul(current_index, np.array(0, dtype=np.int64), op_version=opv), 
                                  np.array([target_size // 2], dtype=np.int64), op_version=opv))
        step.add_to(scope, container)

        for i in range(int(math.log2(target_size))):
            current_index_temp = OnnxAdd(current_index, step, op_version=opv)
            current_index_temp.add_to(scope, container)

            q_current = OnnxGather(quantiles_init, current_index_temp, axis=0, op_version=opv)
            q_current.add_to(scope, container)

            mask = OnnxGreaterOrEqual(flat_input, q_current, op_version=opv)
            mask.add_to(scope, container)

            current_index = OnnxWhere(mask, current_index_temp, current_index, op_version=opv)
            current_index.add_to(scope, container)

            step = OnnxCast(OnnxDiv(step, np.array(2, dtype=np.int64), op_version=opv), 
                           to=TensorProto.INT64, op_version=opv)
            step.add_to(scope, container)

        index_plus_one = OnnxAdd(current_index, np.array(1, dtype=np.int64), op_version=opv)
        ...
        slope = OnnxWhere(invalid_slope, np.array(0, dtype=dtype), slope, op_version=opv)
    ...
    interp2 = create_interpolation(X_neg, quantiles_neg_rev_name, references_neg_rev_name)

    # Combine results
    final_interp = OnnxMul(OnnxSub(interp1, interp2, op_version=opv), 
                          np.array(0.5, dtype=dtype), op_version=opv)
