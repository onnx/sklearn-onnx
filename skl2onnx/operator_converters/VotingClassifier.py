...
from ..common._apply_operation import apply_normalization, apply_mul
...
def convert_voting_classifier(scope, operator, container):    
    ...
    *scikit-learn* enables both modes, transformer and predictor
    ...
    if op.weights is not None:
        ...
        weights = op.weights if isinstance(op.weights, list) else op.weights.flatten().tolist()                
        nbc = n_classes if n_classes > 1 else 2
        values = numpy.zeros((len(weights), nbc))
        ...
        apply_mul(scope, [conc_name, weights_name], weighted_concatenated, container)
