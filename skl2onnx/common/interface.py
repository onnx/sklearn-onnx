...
# This file defines the interface of the converter internal object for
# callback, so the usage of the methods and properties list here will
# not be affected among the different versions.
...
class ModelContainer(metaclass=abc.ABCMeta):
    ...
    def add_initializer(self, name, onnx_type, shape, content):
        ...
        :param onnx_type: Element types allowed in ONNX tensor,
                          e.g., TensorProto.FLOAT and
                          TensorProto.STRING.
        :param shape: Tensor shape, a list of integers.
        :param content: Flattened tensor values (i.e., a float list or
                        a float array).
        """
        return
    ...
    def add_node(self, op_type, inputs, outputs, op_domain='', op_version=1,
                 **attrs):
        """
        Add a NodeProto into the node list of the final ONNX model. If
        the input operator's domain-version information cannot be found
        in our domain-version pool (a Python set), we may add it.

        :param op_type: A string (e.g., Pool and Conv) indicating the
                        type of the NodeProto
        :param inputs: A list of strings. They are the input
...
class ScopeBase(metaclass=abc.ABCMeta):
    pass
